...
class PhotosScheduler:
    ...
    def start(self):
        def make_photos():
            if not self.alarm_controller.alarm_is_started:
                ...
                self.email.send_photos(
                    ...
                    'Raptor: zdjecia wg. harmonogramu z dnia ' + str(photo_date),
                    'W zalaczeniu zdjecia wg. harmonogramu z dnia ' + str(photo_date),
                    photos_to_send)
        ...
        schedule.every().day.at(self.config.schedulers_photo_hour).do(make_photos)
